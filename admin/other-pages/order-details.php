...
<div class="container-fluid">
    <div class="bg-white p-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2 p-3" style="background: rgba(245, 239, 241, 0.7) !important; border-radius: 10px;">
            ...
            <a href="<?= $routes['admin']['orders']['active'] ?>" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-6">
                ...
                <div class="card border-0 shadow-sm" style="border-radius:16px;overflow:hidden;">
                    <div class="card-header" style="background: rgba(245, 239, 241, 0.7) !important; margin: 0 !important; border:none !important; border-top: 2px solid var(--j-border) !important; border-bottom: 2px solid var(--j-border) !important; padding: 1.2rem 1.6rem !important;">
                        <h6 class="mb-0 fw-bold" style="color: #6d3b47 !important;"><i class="fa fa-edit me-2"></i>Update Order Status</h6>
                    </div>
                    <div class="card-body p-4">
                        <form id="updateOrderStatus" enctype="multipart/form-data">
                            <input type="hidden" id="tracking_no" name="tracking_no" value="<?= htmlspecialchars($data['tracking_no']) ?>">
                            <div class="mb-3">
                                <label class="form-label text-muted small">Current Status</label>
                                <select id="order_status" name="order_status" class="form-select" style="border-radius:10px;">
                            ...
                            <button type="button" id="update-Btn" onclick="ValidationCheck();" class="btn w-100" style="background:linear-gradient(135deg,#C97F5F,#b56a4a);color:#fff;border-radius:10px;font-weight:700;padding:.75rem;">
...
<script>
    window.routes = {
        updateOrderStatus: "<?= $routes['admin']['orders']['update-api']; ?>",
    };

    function ValidationCheck() {
        validateForm({
            formId: "updateOrderStatus",
            btn: "update-Btn",
            btnTxt: "Status",
            fields: [
                {id: "order_status", message: "Please Select Order Status.", skipIf: "0"}
            ],
            onSuccess: () => {
                update();
            }
    ...
    function update() {
        submitFormData({
            ...
            btn: "update-Btn",
            btnTxt: "Status",
            url: window.routes.updateOrderStatus,
